...
def trim_message_list(state: GraphState):
    ...
    return {"messages": [RemoveMessage(id=id) for id in removed_ids], "messages_with_think": [RemoveMessage(id=id) for id in removed_ids]}
...
def call_model(state: GraphState):
    ...
    return {"messages_with_think": [response], "id": metadata.get('id'), "model": metadata.get('model_name'), 'finish_reason': metadata.get('finish_reason'),
