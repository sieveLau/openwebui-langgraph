...
class ChromaStore:
    ...
    def __init__(self):
        ...
        self.text_splitter_small = RecursiveCharacterTextSplitter.from_huggingface_tokenizer(
        ...
        self._collection_parent = self._client.get_or_create_collection(self.COLLECTION_NAME_PARENT)
        self._collection_child = self._client.get_or_create_collection(self.COLLECTION_NAME_CHILD)
    ...
    def add_documents(self, documents):
        ...
        expiry_str = (now+timedelta(days=env.get('RAG_DOC_EXPIRE_DAYS'))).strftime(env.get('DATETIME_FMT'))
    ...
    def clean_old_docs(self):
        now = self._get_now()
        self._collection_child.delete(where={"add_ts": {"$lt": ( now - timedelta(days=env.get('RAG_DOC_CLEAN_DAYS_AGO'))).timestamp()}})
        self._collection_parent.delete(where={"add_ts": {"$lt": (now - timedelta(days=env.get('RAG_DOC_CLEAN_DAYS_AGO'))).timestamp()}})
